#!/bin/bash

# Скрипт для сборки Kodi плагина ArjLover

set -e

PLUGIN_NAME="plugin.video.arjlover"
VERSION="1.0.8"
BUILD_DIR="build"
ZIP_NAME="${PLUGIN_NAME}-${VERSION}.zip"

echo "🔨 Сборка плагина ${PLUGIN_NAME} версии ${VERSION}"

# Очистить предыдущую сборку
if [ -d "$BUILD_DIR" ]; then
    echo "🧹 Очистка предыдущей сборки..."
    rm -rf "$BUILD_DIR"
fi

# Создать структуру сборки
echo "📁 Создание структуры сборки..."
mkdir -p "$BUILD_DIR/$PLUGIN_NAME"

# Скопировать необходимые файлы
echo "📋 Копирование файлов плагина..."
cp addon.xml "$BUILD_DIR/$PLUGIN_NAME/"
cp main.py "$BUILD_DIR/$PLUGIN_NAME/"
cp -r resources "$BUILD_DIR/$PLUGIN_NAME/"

# Удалить __pycache__ если есть
find "$BUILD_DIR/$PLUGIN_NAME" -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null || true

# Создать ZIP архив
echo "📦 Создание ZIP архива..."
cd "$BUILD_DIR"
zip -r "$ZIP_NAME" "$PLUGIN_NAME/"
cd ..

# Переместить ZIP в корень проекта
mv "$BUILD_DIR/$ZIP_NAME" .

# Очистить временные файлы
echo "🧹 Очистка временных файлов..."
rm -rf "$BUILD_DIR"

echo "✅ Сборка завершена: $ZIP_NAME"
echo ""
echo "📋 Инструкция по установке:"
echo "1. Откройте Kodi"
echo "2. Перейдите в Настройки → Дополнения"
echo "3. Нажмите 'Установить из ZIP файла'"
echo "4. Выберите файл $ZIP_NAME"
echo "5. Дождитесь уведомления об успешной установке"